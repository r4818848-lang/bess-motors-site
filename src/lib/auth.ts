...
const LEGACY_ADMIN_KEY = "bess-admin-authenticated";
...
/** Unified login: admin password → CRM; otherwise client credentials */
...
export async function registerClient(phone: string, password: string): Promise<AuthResult> {
  ...
  if (password === siteConfig.adminPassword) {
    return loginWithPhonePassword(phone, password);
  }
...
export function getSessionRole(): AuthRole | null {
  ...
  if (localStorage.getItem(LEGACY_ADMIN_KEY) === "1") return "admin";
  return null;
}

export function isAdminAuthenticated(): boolean {
  if (typeof window === "undefined") return false;
  if (localStorage.getItem(LEGACY_ADMIN_KEY) === "1") return true;
