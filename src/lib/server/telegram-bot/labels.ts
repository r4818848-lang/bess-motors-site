...
export const BOT = {
  ...
  monthlyInvoiceParts: "🧾 На фактуру",
  ...
  importBulkWo: "📦 Массовый импорт PDF",
  ...
  helpText:
    "<b>Команды:</b>\n/start — меню\n/find WA12345 — поиск\n\n<b>Быстрый заказ:</b> «📋 Быстрый заказ» → пошагово имя, телефон, авто → сохранение в CRM.\n\n<b>Запчасти:</b> «📦 Запчасти» — название, номер, закуп и продажа <b>брутто</b> (нетто считается сам). «🧾 На фактуру» — отдельный список с тем же заполнением. <b>Расходники:</b> «🧴 Расходники» — только закуп брутто. Таблица + удаление.\n\n<b>Импорт:</b> «📄 Импорт PDF/фото» → PDF → подтвердите. «📦 Массовый импорт» — несколько PDF подряд.\n\n<b>Расходы:</b> «💸 Расходы» → «Показать полный список» — все расходы целиком. Добавить: категория → <code>450 Описание</code>\n\n<b>Механик:</b> <code>/linkmech +48… пароль</code> — привязка Telegram к CRM",
  quickWoIntro:
  ...
  importPrompt:
    "📄 <b>Импорт заказ-наряда</b>\n\nЛучше <b>PDF</b> из Motowarsztat (текст читается точнее).\nФото: отправьте <b>как файл (документ)</b>, не сжатое «фото».\n\nЧитаются клиент, авто, работы и цены продажи (brutto). Закупка подставится автоматически для ZL 16–20, если есть в базе.\n\nСтатус: <b>выдан / оплачен</b>.",
  importBulkPrompt:
    "📦 <b>Массовый импорт</b>\n\nОтправляйте PDF по одному — каждый сразу создаётся в CRM.\nДубликаты (тот же номер ZL) пропускаются.\n\nЛучше PDF, не фото. Для одного файла с проверкой — «📄 Импорт PDF/фото».",
  importCreating: "⏳ Создаю заказ-наряд…",
  importBulkDone: (imported: number, skipped: number, failed: number) =>
    `✅ Готово: создано ${imported}, пропущено ${skipped}, ошибок ${failed}.`,
