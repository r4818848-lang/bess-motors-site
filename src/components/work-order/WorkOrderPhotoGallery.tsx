...
interface Props {
  files: AttachedFile[];
  className?: string;
  variant?: "premium" | "form";
}

function fileImageSrc(f: AttachedFile): string | undefined {
  return f.storageUrl || f.dataUrl;
}

function byCategory(files: AttachedFile[], cats: AttachedFile["category"][]) {
  return files.filter(
    (f) => f.type === "image" && fileImageSrc(f) && cats.includes(f.category)
  );
}
...
export function WorkOrderPhotoGallery({
  ...
  const sections = [
    ...
    ...(isForm ? [] : [{ key: "other", label: p.photosOther, items: other }]),
  ...
  if (sections.length === 0) {
    return (
      <div
        className={
          isForm
            ? `text-center py-6 text-gray-500 text-sm ${className}`
            : `wo-premium-card border-dashed border-bm-border/60 text-center py-10 ${className}`
        }
        onDragOver={(e) => e.preventDefault()}
        onDrop={onDrop}
      >
        {!isForm && <Camera className="w-10 h-10 text-bm-muted mx-auto mb-3 opacity-50" />}
        <p>{p.noPhotos}</p>
      </div>
    );
  }
  ...
  const titleClass = isForm
    ? "text-[11px] font-bold uppercase tracking-wider text-[#e31e24] mb-4 flex items-center gap-2"
    : "wo-premium-section-title mb-6";
  const sectionLabelClass = isForm
    ? "text-[10px] uppercase tracking-wider text-gray-600 mb-3 font-semibold"
    : "text-[10px] uppercase tracking-widest text-bm-silver mb-3";
  const frameClass = isForm
    ? "relative rounded border border-gray-300 overflow-hidden bg-gray-50 aspect-[4/3]"
    : "wo-premium-photo-frame group text-left";

  return (
    <div className={className}>
      <h3 className={titleClass}>
        {!isForm && <Camera className="w-4 h-4 shrink-0" />}
        {p.photosTitle}
      </h3>
      <div className="space-y-6">
        {sections.map((sec, si) => (
          ...
          >
            <p className={sectionLabelClass}>{sec.label}</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {sec.items.map((f, i) => (
                <motion.button
                  ...
                  initial={isForm ? false : { opacity: 0, scale: 0.95 }}
                  animate={isForm ? false : { opacity: 1, scale: 1 }}
                  transition={isForm ? undefined : { delay: i * 0.04 }}
                  whileHover={isForm ? undefined : { scale: 1.03 }}
                >
