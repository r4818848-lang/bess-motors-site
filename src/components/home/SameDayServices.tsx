...
import { Clock, Droplets, Disc, Snowflake } from "lucide-react";
...
export function SameDayServices() {
  ...
  return (
    <section className="py-12 border-t border-bm-border/40" aria-labelledby="same-day-heading">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <ul className="mt-8 grid sm:grid-cols-3 gap-4">
          {items.map((item) => (
            <li
              key={item.source}
              className="rounded-2xl border border-bm-border/60 bg-bm-card/50 p-5 flex flex-col"
            >
              <item.icon className="w-6 h-6 text-bm-red" aria-hidden />
              <p className="mt-3 font-display uppercase text-sm">{item.title}</p>
              ...
              <BookingLink
                ...
                className="btn-primary mt-4 text-xs inline-flex justify-center"
              >
                {h.sameDayBook}
              </BookingLink>
            </li>
