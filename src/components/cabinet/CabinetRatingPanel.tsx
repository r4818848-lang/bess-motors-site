...
export function CabinetRatingPanel({
  ...
  const labels =
    locale === "ru" || locale === "uk"
      ? { title: "Оцените сервис", send: "Отправить", thanks: "Спасибо за отзыв!" }
      : locale === "en"
        ? { title: "Rate our service", send: "Submit", thanks: "Thank you!" }
        : { title: "Oceń serwis", send: "Wyślij", thanks: "Dziękujemy!" };
  ...
  const googleLabel =
    locale === "ru" || locale === "uk"
      ...
      : locale === "en"
        ...
        : "Zostaw opinię w Google Maps";

  if (submitted) {
    return (
      <Card className="p-4 text-center space-y-3">
        <p className="text-bm-red text-sm">{labels.thanks}</p>
        {stars >= 4 && (
          <a
  ...
  return (
    <Card glow className="p-5">
      <p className="font-display uppercase text-sm mb-3">{labels.title}</p>
