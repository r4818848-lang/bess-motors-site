...
export function MaintenanceCalculator({
  ...
  const reminders = useMemo(() => {
    ...
    const loc = useRu ? "ru" : "pl";
    return buildMaintenanceReminders(v, workOrders, loc);
  }, [vehicle, mileage, workOrders, useRu]);
        ...
        {reminders.length === 0 ? (
          ...
          reminders.map((r) => (
            <li key={r.id} className="border-l-2 border-bm-red pl-3">
              <p className="font-medium">{useRu ? r.titleRu : r.titlePl}</p>
              <p className="text-bm-muted text-xs mt-1">{useRu ? r.detailRu : r.detailPl}</p>
            </li>
          ))
        )}
      </ul>
